...
/**
 * Get the number of blocks mined by an account. We disable block subsidy 
 * halving in chainparams.cpp, so this call simply calcaulate block count 
 * as account balance divided by 50.
 * @param request
 * @return 
 */
UniValue getminedblockcount(const JSONRPCRequest& request){
//    JSONRPCRequest request;
//    request.strMethod = "getbalance";
    UniValue balance = getbalance(request);
    int balance_int = int(balance.get_real());
    return UniValue(balance_int/50); 
}
...
{ //  category              name                        actor (function)           argNames
    ...
    { "wallet",  	    "getminedblockcount",       &getminedblockcount,       {} },
};
