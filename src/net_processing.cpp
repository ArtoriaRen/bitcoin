...
void ClientReqLogic::InitializeNode(CNode *pnode) {
    CAddress addr = pnode->addr;
    std::string addrName = pnode->GetAddrName();
    NodeId nodeid = pnode->GetId();
    {
...
void ClientReqLogic::FinalizeNode(NodeId nodeid, bool& fUpdateConnectionTime) {
    fUpdateConnectionTime = false;
    LOCK(cs_main);
    CNodeState *state = State(nodeid);
    assert(state != nullptr);
...
PeerLogicValidation::PeerLogicValidation(CConnman* connmanIn, CPbft* pbftIn, CScheduler &scheduler) : connman(connmanIn), pbft(pbftIn), m_stale_tip_check_time(0) {
...
//        }
	...
	if (idPubkey.first == CPbft::clientID) {
        ...
        pbft->computesSuccessor(p_pbft_block->hash, blockMsg.logSlot);
        pbft->log[blockMsg.logSlot].pPbftBlock = std::move(p_pbft_block);
        int blockPropSuccessor = pbft->log[blockMsg.logSlot].successorBlockPassing;
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (fProcessBLOCKTXN)
            return ProcessMessage(pfrom, NetMsgType::BLOCKTXN, blockTxnMsg, nTimeReceived, chainparams, connman, pbft, interruptMsgProc);
...
bool PeerLogicValidation::SendPPMessages(){
    ...
    if (pbft->isLeader()) {
	    if (pbft->reqQueue.size() >= maxBlockSize
            ...
            pbft->computesSuccessor(p_pbft_block->hash, ppMsg.seq);
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    // Don't bother if send buffer is too full to respond anyway
    if (pfrom->fPauseSend) {
        std::cout << __func__ << ": " << __LINE__ << " ---fPauseSend =  " << pfrom->fPauseSend <<std::endl;
        return false;
    }
    ...
    try
    {
        fRet = ProcessMessage(pfrom, strCommand, vRecv, msg.nTime, chainparams, connman, pbft, interruptMsgProc);
        if (interruptMsgProc)
            return false;
        if (!pfrom->vRecvGetData.empty())
            fMoreWork = true;
...
bool ClientReqLogic::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    const CChainParams& chainparams = Params();
    //
    // Message format
...
bool ClientReqLogic::SendPPMessages() {
    std::cout << "should not call the SendPPMsg of client listening thread." << std::endl;
    return true;
}
